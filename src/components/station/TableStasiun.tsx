...
const TableStasiun = () => {
  ...
  const [currentSelected, setCurrentSelected] = useState<TableStationType>({
    address: '',
    nama_stasiun: '',
    city_id: 0,
    province_id: 0,
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['station', offset],
    queryFn: () =>
      getPaginateStation({ limit: 10, offset, token: token || '' }),
  });
  ...
  return (
    <>
      <Card
        ...
        <CardBody
          className='px-0 overflow-x-auto'
          placeholder={undefined}
          onPointerEnterCapture={undefined}
          onPointerLeaveCapture={undefined}>
